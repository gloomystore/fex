type FetchConfig = Omit<RequestInit, "headers" | "signal"> & {
  headers?: Record<string, string>; // ✅ 자유롭게 헤더 설정 가능
  baseURL?: string;
  url?: string;
  timeout?: number;
...
class FexInstance {
  ...
  interceptors = {
      request: {
          use: (onFulfilled: (config: FetchConfig) => FetchConfig, onRejected?: (error: any) => any) => {
              this.requestInterceptors.push({ onFulfilled, onRejected });
          },
      },
      response: {
          use: (onFulfilled: (response: FexResponse) => FexResponse | Promise<FexResponse>, onRejected?: (error: any) => any) => {
              this.responseInterceptors.push({ onFulfilled, onRejected });
          },
      },
  };

  private requestInterceptors: { onFulfilled: (config: FetchConfig) => FetchConfig, onRejected?: (error: any) => any }[] = [];
  private responseInterceptors: { onFulfilled: (response: FexResponse) => FexResponse | Promise<FexResponse>, onRejected?: (error: any) => any }[] = [];
  ...
  private async request<T = any>(method: string, url: string, data?: any, config: Partial<FetchConfig> = {}): Promise<FexResponse<T>> {
      ...
      const { headers: _, timeout: __, cancelToken: ___, ...restConfig } = mergedConfig;
      const fetchConfig: RequestInit = { headers, signal: controller.signal, ...restConfig };
